...
predicted as VIRUS, which is precisely why NORMAL recall ($0.645$) is the
lowest of the three classes even though NORMAL precision is the highest
($0.993$). In other words, the model almost never calls a sick lung normal
(only $1$ BACTERIA and $0$ VIRUS images are misread as NORMAL), but it
...
(BACTERIA, NORMAL) focus on the relevant regions; the failure case shows a
normal lung misread as viral pneumonia (diffuse activation), the dominant
NORMAL$\rightarrow$VIRUS error. The model produces no clean correctly-classified
VIRUS example, consistent with viral pneumonia being the hardest class.}
\label{fig:gradcam3}
\end{figure}
...
repository, and the trained model weights are published on the Hugging Face
Hub at \url{https://huggingface.co/luanacarolina/pneumonia-chest-xray-classifier}.
