...
\AddToShipoutPictureBG*{%
  \AtPageUpperLeft{%
    \raisebox{-1.2cm}[0pt][0pt]{%
      \makebox[0pt][l]{%
        \hspace*{0.9cm}\includegraphics[width=2.6cm]{logo-ua}%
      }%
...
\begin{abstract}
This project proposes a lightweight deep learning pipeline for classifying pediatric chest X-rays as normal or pneumonia. We use the public Chest X-Ray Images (Pneumonia) dataset (5,863 labeled images) with its imbalanced train/validation/test split. Our methodology compares a custom CNN baseline with transfer learning using ResNet18 and DenseNet121, plus Grad-CAM for qualitative inspection. Evaluation focuses on accuracy, precision, recall, F1-score, AUROC, and inference time, with emphasis on pneumonia recall because false negatives are clinically costly.
\end{abstract}
...
\begin{teaserfigure}
  ...
  \includegraphics[width=0.58\textwidth]{pneumonia}
...
This approach has several technical limitations. The dataset is small and imbalanced, and the validation split is tiny, making model selection noisy. The task is also simplified to binary normal-versus-pneumonia classification, whereas real clinical workflows involve multiple thoracic conditions and uncertain cases. Our experiments also rely on a single public dataset with no external validation, so good in-dataset performance may not transfer to other hospitals, imaging protocols, or adult populations.

\subsection{Responsible Use}
The resulting model should be treated only as an educational prototype or clinician-supervised decision-support tool, not as a stand-alone diagnostic or triage system. Trained on a single pediatric dataset, it cannot confirm or exclude pneumonia in real patients. Any deployment would require multi-site external validation, calibration, bias assessment, clinical oversight, and ethical and regulatory approval.
