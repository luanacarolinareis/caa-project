...
Let me quickly walk you through the structure of our presentation. We'll start with the Outline, then move into Motivation and the clinical problem, the Dataset, Preprocessing, and the Pipeline and Model Architectures. From there we'll cover the Training Setup and then hand over to Carolina for the results --- Quantitative Results, a Visual Comparison of the models, and the Training Curves for our best model, DenseNet121. The next slide shows topics ten through eighteen.
}
...
Our second innovation is a hierarchical two-stage classifier, motivated by the original Kermany et al.\ 2018 paper --- which, remarkably, \textbf{never trained a flat three-way classifier}, but rather used two separate binary classifiers. We replicate that design philosophy and compare it head-to-head with our flat softmax approach.
...
To summarise what we built and what we found.

Our \textbf{binary ensemble} achieves F1 of 0.896, recall of 0.997, and AUROC of 0.963 --- missing only 1 pneumonia case out of 390. Transfer learning substantially outperforms training from scratch across every metric, while inference cost is identical at roughly 13 to 15 milliseconds.

Our \textbf{three-class innovation} reaches macro-F1 of 0.808 and AUROC of 0.960 on a leakage-free patient-aware split --- demonstrating that the aetiology signal encoded in the filenames is genuinely learnable with the same lightweight architecture.

The \textbf{hierarchical classifier} does not beat the flat softmax, which tells us the Virus bottleneck is a data-level problem, not a modelling one.

Grad-CAM confirms that both transfer learning models attend to clinically plausible lung regions, and the three-class ensemble achieves the best calibration of all models at ECE approximately 0.05.

For future work, the clearest directions are: addressing class imbalance with weighted or focal loss, temperature scaling for calibration, multi-institution and adult datasets, severity grading, and eventually prospective clinical evaluation. Thank you.
}
